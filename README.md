...
- **タグが付くと、Discordの返信にも `タグ：#運動 #健康` の行が出ます。** タグ無しで保存された場合はこの行が出ないので、動いているかどうかがその場で分かります
- **タグ付けは保存の前に行われるため、そのぶん返信が遅くなります。** ローカルLLM（`ollama`）の場合、しばらく投稿が無かったあとの1件目は**モデルをメモリに読み込む時間**が加わり、1分近くかかることがあります（2件目以降は数秒です）。上限は `TAGGING_TIMEOUT_SECONDS`（既定120秒）で、超えるとタグ無しで保存されます
- **タイムアウトしたかどうかはログに残ります。** `タグ付けがタイムアウトしました（120秒）` の行を探してください
...
6. `TAGGING_ENABLED=true` にしている場合は、返信に `タグ：#運動 #健康` の行が付くことを確認します（付かない場合はタグ無しで保存されています）。
