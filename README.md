...
TAGGING_TIMEOUT_SECONDS=30    # タグ生成を待つ上限秒数
...
| `TAGGING_TIMEOUT_SECONDS` | `30` | AIの応答を待つ上限 |
...
- **タグ付けは保存の前に行われます。** そのぶん返信が数秒〜十数秒遅くなります。遅すぎる場合は `TAGGING_TIMEOUT_SECONDS` を短くしてください（タイムアウトしてもタグ無しで保存されます）
