...
erDiagram
    ...
    profiles ||--o{ comments : ""
    profiles ||--o{ notifications : "受け取る人"
    profiles ||--o{ notifications : "actor_id"
    activity_posts ||--o{ notifications : "任意"
    groups   ||--o{ notifications : "任意"
...
| `notifications`                   | 追加（`0010`）                                                                                     | アプリ内のお知らせ。行はトリガーが作り、利用者は INSERT できない  |
| `profiles.notify_*`               | 追加（`0010`）                                                                                     | 種類ごとに受け取りを止められるようにするため                      |
...
7. お知らせは本人しか読めない。行を作るのはトリガー（`SECURITY DEFINER`）だけで、
   利用者側に INSERT のポリシーを与えない。他人あてのお知らせを作れる経路を残さないため。
8. 写真も `can_view_post()` を通す。Storage 側の読み取りポリシーが
...
| お知らせ                               | 自分あての応援・コメント・グループ参加                         | 10       |
| 設定                                   | 表示名・画像・タイムゾーン・既定公開範囲・お知らせ・ログアウト | 1, 4, 10 |
...
| `/notifications`         | 必要                     | お知らせ（Phase 10 で追加）                  |
| `/settings`              | 必要                     | 設定                                         |
...
| 10    | アプリ内のお知らせ（応援・コメント・グループ参加、種類ごとの切り替え）     | **完了** |
...
| 通知が「見られている」圧力になる             | プロダクト思想と矛盾           | 知らせるのは自分に向けられたものだけ。他人の記録が増えても鳴らさない。種類ごとに切れる。赤い点や大きな数字を使わない |
| 通知の作成漏れ・二重作成                     | 届かない／うるさい             | 行はアプリのコードではなく DB のトリガーが作る。応援は部分一意インデックスで「未読1件」に束ねる                      |
...
- **お知らせの行はアプリではなくトリガーが作ります。** 応援やコメントを作る経路が
  増えたときに、片方だけ通知されない事故を防ぐためです。
  他人あての行を作るので `SECURITY DEFINER` にし、利用者側には INSERT のポリシーを与えていません。
- **応援は「未読1件」にまとめます。** `(user_id, post_id) where type='reaction' and read_at is null`
  の部分一意インデックスを置き、2人目からは行を増やさず人数だけを足します。
  既読にしたあとの応援は、新しいお知らせとして出します。
- **コメントはまとめません。** 誰への返事か分からなくなるためです。
- **画面を開いただけでは既読にしません。** 眺めただけのものが消えると読み返せません。
  「すべて既読にする」を明示的に押してもらいます。
- **未読は左端の線で示します。** 赤い点や大きな数字は、急かす方向に働くため使いません。
  ベルの数字も9件で頭打ちにしています。
- **下部ナビゲーションは触っていません。** すでに5つで埋まっており、
  6つ目を足すと片手で押しづらくなります。ベルは画面上部に置きました。
