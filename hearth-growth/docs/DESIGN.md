...
erDiagram
    ...
    activity_posts ||--o{ post_groups : "visibility = group（複数可）"
    groups   ||--o{ post_groups : ""
...
| `activity_posts` の CHECK         | ~~`visibility = 'group'` のときだけ `group_id` を持つ~~（`0012` で廃止）                           | 公開先を複数持てるようにしたため、整合は RPC 側で見る              |
| `post_groups`                     | 追加（`0012`）。`activity_posts.group_id` は廃止                                                   | 1つの記録を複数のグループへ公開できるようにするため                |
| `categories.counts_toward_total`  | 追加（`0014`）                                                                                     | 睡眠のように、記録は残したいが努力量ではないものを集計から外すため |
...
| 11    | 招待リンクの不具合修正（トークンを base64url にそろえる）                  | **完了** |
| 12    | 「自分だけ」の記録に気づける導線と、まとめて公開する操作                   | **完了** |
| 13    | タイムラインが空になっていた不具合の修正（埋め込み select の曖昧さ）       | **完了** |
| 14    | 複数グループへの公開・グループの削除・就寝／起床のショートカット           | **完了** |
| 15    | 画面表示の不具合修正（向きの固定・狭い画面からの導線）                     | **完了** |
| 16    | 就寝・起床の時刻表示と、起床予定の呼びかけ（Web Push）                     | **完了** |
...
### Phase 16 の実装メモ（就寝・起床の時刻と、起床の呼びかけ）

**時刻の表示**

- 開始と終了は以前から `activity_posts.started_at / ended_at` に入っていた。
  画面に出していなかっただけなので、出すようにした。
- 睡眠は日付をまたぐ。「23:30 → 7:05」だけでは
  7時間半か16時間半か読み取れないので、日が変わっていれば「翌」を添える。

**アラームについて（できないことを先に書く）**

- **Web アプリに、音で確実に起こす手立ては無い。**
  端末のアラームへ登録する API は存在せず、
  時刻指定の通知（Notification Triggers）は Chrome で試験導入されたが製品版に入らなかった。
- Web Push は「その時刻ごろ」に出せるが、省電力で遅れることがあり、
  マナーモードも越えず、鳴り続けもスヌーズも無い。
- そこで**起こす役目は端末のアラームに任せ**、
  こちらは「起きたことを1タップで記録する」ための呼びかけに徹する。
  画面にもその旨を書いている。曖昧にすると寝過ごしてから気づくことになる。

**通知の作り**

```
pg_cron（1分ごと）→ pg_net → /api/push/dispatch
  → claim_due_wake_alarms(合言葉) で宛先を取得 → Web Push
通知の「起きている」→ サービスワーカーが /api/sleep/wake を叩く → wake_up()
```

- **サービスロールキーは使わない。** これまでどおりの方針を通すため、
  定期実行に開けるのは合言葉で守った関数1つだけにした。
  返るのは通知の宛先だけで、記録の中身は通らない。
  合言葉は `app_config` に置き、**誰も読めないポリシー（`using (false)`）**を張った。
- 取り出した時点で `notified_at` を立てるので、同じ予定を二度送らない。
  確認用には別の関数（`check_cron_secret`）を用意した。
  `claim_*` を確認に使うと、時刻の来た予定をそこで消費してしまう。
- 「起きている」はアプリを開かずに終える。
  サービスワーカーからの同一オリジンの要求なので、ログインの手形がそのまま届く。
  届かなければアプリを開く動きに切り替える。
- タイマーが終わったら予定も消す（トリガー）。
  残すと、寝ていないのに呼びかけが飛ぶ。
- PostgREST に対して実際に確かめた: 匿名の鍵では
  `app_config` も `sleep_alarms` も `push_subscriptions` も1行も読めず、
  合言葉が違えば `claim_due_wake_alarms` は `forbidden` を返す。

### Phase 15 の実装メモ（画面表示の不具合）

**タブレットを横にすると画面いっぱいにならなかった**

- マニフェストに `"orientation": "portrait"` を書いていました。
  ホーム画面から起動すると Android が縦のまま固定し、
  横にしても縦の窓が中央に置かれて左右に壁紙が見えていました。
- **端末の向きは利用者が決めることで、アプリが決めることではありません。**
  指定ごと外しました。E2E で「向きを固定していない」ことを見張ります。
- マニフェストはサービスワーカーが precache しているので、名前も `v2` へ上げました。

**スマートフォンの縦画面からグループへ行けなかった**

- 横のナビ（`SideNav`）は幅が広いときだけ出ます。
  下部ナビは5つで埋まっているため、`/groups` は
  **狭い画面からどこにも導線がありませんでした。**
- マイページに「そのほか」を置き、グループ・お知らせ・設定へ行けるようにしました。
- 画面を足したときに同じことが起きないよう、
  `src/tests/mobile-reachability.test.ts` が
  「`(app)` 配下の画面がすべて下部ナビか『そのほか』から開けるか」を機械的に見ます。
  例外は理由を書いて明示します（`/timer/finish` はタイマー終了後に自動で開くため）。
  グループの導線を消すと落ちることを確認しました。

### Phase 14 の実装メモ（公開先の複数化・グループ削除・睡眠）

**公開先を複数に（0012）**

- `activity_posts.group_id` を廃止し、`post_groups` へ移しました。
  `post_allowed_users`（「選んだ人」の宛先）と同じ形にそろえています。
- 付け替えは RPC だけに許し、利用者に insert のポリシーを与えていません。
  自分の記録を勝手なグループへ結び付けられないためです。
- 公開先が0件になったら本体を「自分だけ」へ戻します（`post_groups_normalize`）。
  「グループ公開なのに誰にも届かない」状態を残さないためです。

**グループの削除（0013）**

- **実装前に危ないことに気づきました。** `activity_posts.group_id` は
  `on delete cascade` だったので、グループを消すと
  **そこに公開されたメンバー全員の記録が道連れで消えていました。**
  0012 で列ごと無くなったため、この経路は塞がっています。
- 消えるのは入れ物だけ。記録は誰のものであっても残します。
  「グループを削除しても記録は1件も消えない」をテストで固定しました。
- グループのカテゴリーを使っている記録は、削除の前に
  持ち主の個人カテゴリーへ移します。`categories.group_id` は cascade、
  `activity_posts.category_id` は restrict なので、
  そのまま消すと外部キー違反で失敗するためです。
- 取り消せない操作なので、画面ではグループ名を打ってもらってから実行します。

**就寝・起床（0014）**

- 仕組みを増やさず、既にあるタイマーへ寄せました。
  就寝は `start_session`、起床は `complete_session` + 記録の作成にあたります。
  経過時間はサーバーの時刻で測られ、端末の時計に左右されません。
- **睡眠は活動時間の合計に数えません。** 8時間の睡眠を足すと、
  1時間の勉強が数字の上で埋もれます。
  カテゴリーに `counts_toward_total` を持たせ、集計側で外しています
  （今日・今週・カテゴリー別・連続記録・グループの週次すべて）。
- 連続記録も睡眠では伸びません。寝ただけで「続いている」ことにすると、
  この数字の意味が無くなります。
- 押すのは1回ずつ。寝る前と寝起きに文章を書かせません。

### Phase 13 の実装メモ（タイムラインが空だった不具合）

タイムラインには**当初から1件も出ていませんでした**。原因は埋め込み select です。

```
PGRST201: Could not embed because more than one relationship was found
for 'activity_posts' and 'profiles'
  ① activity_posts_user_id_fkey（多対一・投稿者）
  ② post_allowed_users を挟んだ多対多（「選んだ人」の宛先）
```

`post_allowed_users` は 0001 からある中間テーブルなので、
`profiles(...)` はずっと曖昧なままでした。

**気づけなかった理由が2つあります。両方直しました。**

1. **型検査でもビルドでも分かりません。** どの道を通れるかはデータベースの
   外部キーで決まるためです。TypeScript は `profile` の型を正しく解決してしまいます。
   → `scripts/check-selects.mjs` を追加し、本物の PostgREST に同じ select を投げて
   確かめるようにしました（`npm run db:selects`）。
   わざと `!user_id` を外すと落ちることも確認しています。
2. **アプリが失敗を空配列に潰していました。** 画面には「まだ投稿がありません」と出るため、
   利用者にも開発側にも「そもそも投稿が無い」ようにしか見えませんでした。
   → `TimelinePage.failed` を足し、「読み込めませんでした」を別の文言で出します。

読み込み失敗を空で返すのは、20章「エラーを握り潰さない」に反していました。

### Phase 12 の実装メモ（見えない記録に気づけるように）

グループに入る前に記録すると、公開範囲は「自分だけ」になります
（グループが無いので `group` を選べないため）。あとから参加しても、
その記録は自動では共有しません。**勝手に公開範囲を広げてはいけない**からです。

ただしこの仕様は、**「参加したのに仲間から見えない」として現れます**。
実際にそうなりました。仕様どおりでも、気づけなければ不具合と同じです。

- **記録の画面で件数を伝えます。**「◯件の記録が『自分だけ』のままです」。
- **まとめて公開できるようにしました。** 1件ずつ開き直させるのは現実的ではありません。
- ただし公開範囲を広げる操作なので、囲いを3つ置いています。
  - 動かすのは `private` のものだけ。`selected` の宛先指定には触れない
  - 画面に出した件数と食い違ったら実行しない（その間に増えたものを巻き込まない）
  - 所属していないグループへは出せない（RLS の `with check`。一括更新でも効くことをテスト済み）
- 実行前に、件数と公開先の名前を出して確認します。押すまで何も起きません。

### Phase 11 の実装メモ（招待リンクの不具合）

招待リンクが**一度も機能していませんでした**。原因は2つが重なったことです。

1. トークンは `+` `/` を `-` `_` に置き換えていたものの、
   base64 の詰め物（末尾の `=`）を落としていませんでした。
   32byte の乱数は必ず `=` 1個で終わるため、リンクは常に `.../invite/xxx=` の形でした。
2. **Next.js は経路の `=` を復号せず `%3D` のまま渡します。**
   `%2D` は `-` に復号されるのに `=` はされない、という非対称があります。
   結果、画面に届く時点で `%` を含む文字列になり、形式検査で必ず弾かれていました。

- **base64url が詰め物を持たない仕様なのは、まさにこれを避けるためです。**
  0011 で既定値を `translate(..., '+/=', '-_')` に変え、既存の行からも `=` を落としました。
- **受け取り側でも符号化を戻し、詰め物を落としてから検査します**（`normalizeInvitationToken`）。
  これで、すでに配ってしまった `=` 付きのリンクもそのまま使えます。
- **復号は検査の前に行い、検査は復号後の文字列に掛けます。**
  順序を逆にすると `%2E%2E%2F` が素通りします。この順序はテストで固定しています。
- **この不具合は単体テストでは取り逃しました。** スキーマ単体では `=` 付きも通るためです。
  実際に URL を叩く E2E（`e2e/invite-link.spec.ts`）を足し、
  修正前のコードに戻すと落ちることを確認しています。
