...
| `sleep_alarms`                 | 本人のみ                                                                                                                  | **不可**（start_sleep だけ）                        | 不可                               | 不可                                               |
| `app_config`                   | **不可**（合言葉を置く場所。定義者権限の関数だけが読む）                                                                  | 不可                                                | 不可                               | 不可                                               |
...
| `claim_due_wake_alarms(secret)`          | 定期実行から通知の宛先だけを取り出すため。サービスロールキーを配らずに済ませる代わりに、合言葉で守る              |
| `check_cron_secret(secret)`              | /setup-check 用。claim_* を確認に使うと、時刻の来た予定をそこで消費してしまう                                     |
| `start_sleep(wake_at?)` / `wake_up()`    | 就寝・起床を1タップにするため。睡眠カテゴリーの用意とタイマーの開始・終了・記録化をまとめて行う                   |
