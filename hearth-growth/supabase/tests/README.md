...
- 1つの記録を複数のグループへ公開できる／公開先でないグループの人には見えない
- グループを削除しても記録は1件も消えない／公開先が無くなった記録は「自分だけ」に戻る
- 就寝・起床で睡眠時間が残る／睡眠は活動時間・カテゴリー別・連続記録に数えない
- 起床予定を登録できる／過去や24時間より先は受け付けない
- 通知の宛先と起床予定が本人にしか見えない
- 合言葉が合わなければ通知先を1件も取り出せない／同じ予定を二度送らない
